...
data_match = re.search(r'export const awsServices: Service\[\] = (\[[\s\S]*?\n\]);', content)
data = json.loads(data_match.group(1))

# Create new service entry
new_svc = {
    'name': service_name,
    'description': 'Amazon Cognito Identity enables you to create unique identities for your users and authenticate them with federated identity providers.',
    'commands': []
}
...
for i, cmd in enumerate(all_cmds, 1):
    ...
    new_svc['commands'].append({'name': cmd, 'description': f'{cmd} command', 'parameters': params})
    print(f"✓ {len(params)} params")

data.append(new_svc)
data.sort(key=lambda x: x['name'])

print(f"\n✅ Added: {len(new_svc['commands'])} commands")

shutil.copy('lib/aws-commands.ts', '.dev-data/aws-commands.backup-before-cognito-identity.ts')
