...
print("🔧 Fixing EFS\n")
session = Session()
client = session.create_client('efs', region_name='us-east-1')
...
idx = next(i for i, s in enumerate(data) if s['name'] == 'efs')
svc = data[idx]
...
shutil.copy('lib/aws-commands.ts', '.dev-data/aws-commands.backup-before-efs-fix.ts')
