...
def run_aws_help(service, command):
    try:
        result = subprocess.run(['aws', service, command, 'help'], capture_output=True, text=True, 
                              timeout=20, env={**os.environ, 'AWS_PAGER': ''})
        return re.sub(r'(.)\x08\1', r'\1', result.stdout)
    except Exception as e:
        print(f"Error running help for {command}: {e}")
        return ""
...
def parse_parameters(help_text):
    ...
    if cur: params.append(cur)
    for p in params: 
        p['description'] = re.sub(r'\s+', ' ', p['description']).strip()[:247]
    return params

print("🔧 Fixing IAM Service Data\n")
session = Session()
service_name = 'iam'
...
idx = next(i for i, s in enumerate(data) if s['name'] == service_name)
svc = data[idx]
...
for i, cmd in enumerate(all_cmds, 1):
    if cmd not in existing or not existing[cmd].get('parameters') or len(existing[cmd]['parameters']) == 0:
        print(f"[{i}/{len(all_cmds)}] {cmd}...", end=' ', flush=True)
        help_txt = run_aws_help(service_name, cmd)
        params = parse_parameters(help_txt) if help_txt else []
        new_commands.append({
            'name': cmd,
            'description': f'{cmd} command' if cmd not in existing else existing[cmd].get('description', f'{cmd} command'),
            'parameters': params
        })
...
# Backup
shutil.copy('lib/aws-commands.ts', '.dev-data/aws-commands.backup-before-iam-fix.ts')
