...
print("🔧 Fixing ECR\n")
session = Session()
client = session.create_client('ecr', region_name='us-east-1')
...
idx = next(i for i, s in enumerate(data) if s['name'] == 'ecr')
svc = data[idx]
...
shutil.copy('lib/aws-commands.ts', '.dev-data/aws-commands.backup-before-ecr-fix.ts')
